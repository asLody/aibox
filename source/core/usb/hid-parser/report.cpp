...
bool ExtractAsUnit(const uint8_t* report,
    ...
    if (val < static_cast<double>(attr.logc_mm.min) ||
        val > static_cast<double>(attr.logc_mm.max)) {
        return false;
    }
    ...
    double resolution = static_cast<double>(logc_max - attr.logc_mm.min) /
                        static_cast<double>(phys_max - phys_min);
...
bool InsertUint(uint8_t* report,
                size_t report_len,
                const hid::Attributes& attr,
                uint32_t value_in) {
...
bool InsertAsUnit(uint8_t* report,
                  size_t report_len,
                  const hid::Attributes& attr,
                  double value_in) {
    // If the minimum value is less than zero, then the maximum
