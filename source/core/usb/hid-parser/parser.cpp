...
template <typename T> class Fixup {
public:
...
public:
    ...
    void erase(size_t index) { values_.erase(values_.begin() + index); }
...
class ParseState {
public:
    static char* Alloc(size_t size) { return new char[size]; }
    ...
    bool Init() {
        ...
        report_ids_.push_back(empty_report_id);
    ...
    ParseResult Finish(DeviceDescriptor** device) {
        ...
        if (mem == nullptr) return kParseNoMemory;
        ...
        for (const ReportID& report_id : report_ids_.values()) {
            ...
            for (const auto& f : fields_.values()) {
                if (f.report_id != report_id.report_id) continue;

                switch (f.type) {
                    case NodeType::kInput:
                        if (input_count == report_id.input_count) return kParseOverflow;
                        input_fields_start[input_count] = f;
                        input_fields_start[input_count].col = coll_fixup(f.col);
                        input_fields_start[input_count].attr.offset =
                                static_cast<uint32_t>(input_bit_sz);
                        input_bit_sz += f.attr.bit_sz;
                        ++input_count;
                        break;
                    case NodeType::kOutput:
                        if (output_count == report_id.output_count) return kParseOverflow;
                        output_fields_start[output_count] = f;
                        output_fields_start[output_count].col = coll_fixup(f.col);
                        output_fields_start[output_count].attr.offset =
                                static_cast<uint32_t>(output_bit_sz);
                        output_bit_sz += f.attr.bit_sz;
                        ++output_count;
                        break;
                    case NodeType::kFeature:
                        if (feature_count == report_id.feature_count) return kParseOverflow;
                        feature_fields_start[feature_count] = f;
                        feature_fields_start[feature_count].col = coll_fixup(f.col);
                        feature_fields_start[feature_count].attr.offset =
                                static_cast<uint32_t>(feature_bit_sz);
            ...
            if (input_count == 0) input_bit_sz = 0;
            if (output_count == 0) output_bit_sz = 0;
            if (feature_count == 0) feature_bit_sz = 0;
    ...
    ParseResult start_collection(uint32_t data) {  // Main
        if (!is_valid_collection(data)) return kParseInvalidItemValue;

        if (coll_size_ >= kMaxCollectionCount) return kParseOverflow;
        ...
        uint32_t usage = usages_.values().empty() ? 0 : usages_.values()[0];

        Collection col{static_cast<CollectionType>(data),
    ...
    ParseResult end_collection(uint32_t data) {
        if (data != 0u) return kParseInvalidItemValue;
        if (parent_coll_ == nullptr) return kParseInvalidTag;
    ...
    ParseResult add_field(NodeType type, uint32_t data) {  // Main
        if (coll_size_ == 0) return kParseUnexpectedItem;

        if (!validate_ranges()) return kParseInvalidRange;
        ...
        for (uint32_t ix = 0; ix != table_.report_count; ++ix) {
            ...
            ReportField field{report_ids_.values()[table_.report_ids_index].report_id,
                              attributes,
                              type,
                              flags,
                              curr_col};
    ...
    ParseResult set_usage_min(uint32_t data) {  // Local
        if (data > UINT16_MAX) return kParseUnsuported;
    ...
    ParseResult set_usage_page(uint32_t data) {  // Global
        if (data > UINT16_MAX) return kParseInvalidRange;
    ...
    ParseResult set_report_id(uint32_t data) {  // Global
        if (data == 0) return kParserInvalidID;
        if (data > UINT8_MAX) return kParseInvalidRange;

        // It's an error to define a report id after items have been defined with no id.
        if (report_id_has_items(report_ids_[0])) return kParserMissingID;
        ...
        report_ids_.push_back(new_report_id);
    ...
    ParseResult push(uint32_t data) {  // Global
        stack_.push_back(table_);
        return kParseOk;
    }

    ParseResult pop(uint32_t data) {  // Global
        if (stack_.size() == 0) return kParserUnexpectedPop;
...
private:
    ...
    public:
        ...
        Usage next_usage() {
            ...
            if (usages_ == nullptr) {
                ...
                if (usage_long > usage_range_.max) usage_long = usage_range_.max;
            } else {
    ...
    bool validate_ranges() const {
        if (usage_range_.min > usage_range_.max) return false;
        if (table_.attributes.logc_mm.min > table_.attributes.logc_mm.max) return false;
        return true;
    }
...
ParseResult ParseReportDescriptor(const uint8_t* rpt_desc,
                                  size_t desc_len,
                                  DeviceDescriptor** device) {
    impl::ParseState state;

    if (rpt_desc == nullptr || device == nullptr) return kParseMoreNeeded;

    if (!state.Init()) return kParseNoMemory;
    ...
    while (len > 0) {
        ...
        if (actual > len) return kParseMoreNeeded;
        if (actual == 0) return kParseUnsuported;

        auto pr = ProcessItem(item, &state);
        if (pr != kParseOk) return pr;
...
Collection* GetAppCollection(const ReportField* field) {
    ...
    while (collection != nullptr) {
        if (collection->type == hid::CollectionType::kApplication) break;
